...
> save routine *invokes* that skill as the single source of truth for line caps, the decision table,
> and token awareness, so the "commit what we learned" step can't quietly undo your config hygiene.
...
- **Carry the cost constraints forward — by reference, not by copy.** It invokes the standards skill
  as the single source of truth for the line cap and token-per-turn framing, so the save step can't
  quietly undo the config hygiene *and* the rules can't drift out of sync.
...
- **Invoke the `claude-md-and-folder-standards` skill and apply it as the single source of truth**
  for sizing and placement rules — don't restate them here (an inlined copy drifts).
- Highlights only: CLAUDE.md ≤200 lines; skills ≤120; add only what Claude would get WRONG
  without; no duplicating existing files; terse imperative bullets.
