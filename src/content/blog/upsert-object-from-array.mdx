...
A quick solution to this problem is to use the [update](https://docs.mulesoft.com/dataweave/latest/dw-operators#update-operator)[operator](https://docs.mulesoft.com/dataweave/latest/dw-operators#update-operator) because it has an upsert option to it. Let's see how to do this.
...
Issue [#1](https://www.prostdev.com/blog/hashtags/1) is happening because we are updating the field *name* with this: *updateUser.name* - and since this field does not exist, it's returning a *null* value.
...
Issue [#2](https://www.prostdev.com/blog/hashtags/2) is where we will be using our upsert operator because we do not have this field in our original value, so, we have to tell the operator that we want it to insert it if it doesn't exist. We do that by adding an exclamation point (!) right next to the field. Like so:
