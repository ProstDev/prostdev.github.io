...
faqs:
  - question: "How do I check whether my Runtime Fabric cluster is up and running?"
    answer: "There are two ways the article describes: via the Anypoint Platform console under Runtime Manager -> Runtime Fabrics, where you get a summary plus Application Status, Deployments, and Nodes detail; or via the `rtfctl` CLI by running `sudo rtfctl status`, which returns the cluster state and the health of the controller and worker nodes. A healthy cluster shows the Active status, while problems show up as Degraded or disconnected."
  - question: "How do I install the rtfctl CLI?"
    answer: "Download it for your platform with a curl command from the Anypoint download endpoint: `rtfctl-windows/latest` for Windows, `rtfctl-darwin/latest` for MacOS, and `rtfctl/latest` for Linux. Then make it available in your PATH and you can start using it; the author normally installs rtfctl within the controller nodes of the Runtime Fabric cluster. The official documentation is at https://docs.mulesoft.com/runtime-fabric/latest/install-rtfctl."
  - question: "Why does my Runtime Fabric cluster go into a Degraded state?"
    answer: "A cluster turns Degraded when it cannot reach the endpoints it needs to connect to the control plane, which is common for enterprise deployments behind a corporate firewall. You can validate connectivity with `sudo ./rtfctl test outbound-network`, which checks each required endpoint; if any of them are not responding the cluster changes into a Degraded state. The required endpoints are documented at https://docs.mulesoft.com/runtime-fabric/1.3/install-port-reqs under Port IPs and Hostnames to Add to the Allowlist."
  - question: "How do I get the logs from an application running on Runtime Fabric?"
    answer: "First get the namespace of the environment where you deployed the application with `kubectl get namespaces` (a namespace maps to an Anypoint environment such as Sandbox or Production), then list the pods with `kubectl get pods -n <namespace>`, and finally retrieve the runtime log with `kubectl logs deploy/<app-name> -n <namespace> -c app`. To watch the log live like a tail, add `--follow` to that command. The app-name is the name you chose when deploying, which matches the pod name without the random ID Kubernetes appends."
  - question: "What happens behind the scenes after I click the deploy button for an RTF application?"
    answer: "Several things happen: first the application artifact is registered on Exchange, then a Docker-based image is uploaded to the AWS image repository, then the Runtime Fabric creates the deployment using that image, and finally the application is deployed as a pod containing two containers, one holding the MuleSoft Runtime with your application and a second sidecar container for monitoring. At the RTF level you will see the pod initialize and then transition to a Running state once both containers are READY."
  - question: "How do I get a shell inside a running pod on Runtime Fabric?"
    answer: "Run `kubectl exec --stdin --tty <podname> -n <namespace> -- /bin/bash` using the full pod name, and the command prompt returns you inside the pod. From there you can run `ps -fea` to see the Java process running your MuleSoft Runtime or browse the apps folder to see your application, but you cannot write or modify anything; to change something like JVM properties you do it from the console, which creates a new pod with that configuration."
---
